<?php
    class userModel{
        ...
        function changeAdmin($idUser,$permiso){
            $query = $this->db->prepare(
            'UPDATE usuarios SET permiso = ? WHERE id_usuario = ?');
            $query->execute([$permiso,$idUser]);
        }

        function getAllAdmin(){
            $query = $this->db->prepare(
                'SELECT COUNT(id_usuario) AS cant FROM `usuarios` WHERE permiso = 1');
                $query->execute([]);
                return $query->fetch(PDO::FETCH_OBJ);
        }
        ...
        function isAdmin($idUser){
            $query = $this->db->prepare(
            'SELECT * FROM `usuarios` WHERE permiso = 0 && id_usuario = ?');
            $query->execute([$idUser]);
            return $query->fetch(PDO::FETCH_OBJ);
        }
        ...
        function getPermiso($idUser){
            $query = $this->db->prepare(
            'SELECT permiso FROM `usuarios` WHERE id_usuario = ?');
            $query->execute([$idUser]);
            return $query->fetch(PDO::FETCH_OBJ);
        }
